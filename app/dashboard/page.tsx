...
export default async function DashboardPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 space-y-8">
      {/* Boutons d'accès rapide pour Conseiller / Admin */}
      {(showConseillerLink || showAdminLink) && (
                ...
                devisList.map((devis) => (
                  <TableRow key={devis.id} className="hover:bg-muted/40">
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-xs px-2.5 py-0.5 font-medium ${statutDevisColors[devis.statut] || ""}`}
                      >
