...
import { CheckCircle2, CalendarCheck, FileText, ArrowRight, MapPin, Users } from "lucide-react";
  ...
  return (
    <main className="max-w-3xl mx-auto py-12 px-4 space-y-8 text-center sm:text-left">
      ...
      <div className="flex flex-col items-center sm:items-start gap-4 p-6 rounded-2xl bg-emerald-50 border border-emerald-200 dark:bg-emerald-950/30 dark:border-emerald-900">
        ...
        <div className="space-y-1">
          ...
          <p className="text-emerald-700 dark:text-emerald-400 text-sm">
            Nous avons bien pris en compte votre règlement pour la réservation #{reservation.id}.
            Un e-mail de récapitulatif vous a été envoyé.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="border-border/60">
          ...
          <CardContent className="space-y-3 text-sm">
            <div>
              <span className="text-muted-foreground text-xs uppercase tracking-wider block">Intitulé</span>
              <p className="font-bold text-foreground">{circuitTitle}</p>
            </div>

            {reservation.nbVoyageurs > 0 && (
              <div>
                <span className="text-muted-foreground text-xs uppercase tracking-wider block">Voyageurs</span>
                <p className="font-medium">{reservation.nbVoyageurs} personne(s)</p>
              </div>
            )}

            {reservation.dateDebut && (
              <div>
                <span className="text-muted-foreground text-xs uppercase tracking-wider block">Dates</span>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            {lastTransaction && (
              <>
                ...
                <div>
                  <span className="text-muted-foreground text-xs uppercase tracking-wider block">Statut Transaction</span>
                ...
                {lastTransaction.providerRef && (
                  <div>
                    <span className="text-muted-foreground text-xs uppercase tracking-wider block">Référence</span>
      ...
      <Card className="border-primary/20 bg-primary/5">
        <CardHeader>
          <CardTitle className="text-base font-bold text-foreground">Prochaines Étapes</CardTitle>
        </CardHeader>
        <CardContent className="text-sm space-y-2 text-muted-foreground">
          <p>1. Votre conseiller dédié examine votre dossier de réservation.</p>
          <p>2. Vous recevrez une convocation et le carnet de voyage avant votre départ.</p>
          <p>3. Pour toute question, vous pouvez contacter notre équipe à tout moment.</p>
        </CardContent>
      </Card>

      <div className="flex flex-wrap justify-center sm:justify-start gap-4 pt-2">
        <Link href={`/reservations/${reservation.id}`} className={buttonVariants({ variant: "default" })}>
